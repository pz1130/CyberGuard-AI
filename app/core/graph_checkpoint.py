...
resume once an admin decides — the decision arrives in the API process, the run
lives in a Celery worker.
...
Callers that create a new event loop per task (Celery does) must use the
context-manager form so the pool is opened and closed on that loop.
"""
...
async def open_checkpointer() -> AsyncIterator[Any]:
    ...
    Always opens a fresh pool (or a fresh MemorySaver) so Celery's "new loop
    per task" pattern cannot leave us holding a pool tied to a closed loop.
    """
    ...
    try:
        ...
        if pool is not None:
            try:
                await pool.close()
            except Exception as e:  # noqa: BLE001 - teardown, not a security path
