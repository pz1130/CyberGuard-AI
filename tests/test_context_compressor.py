...
def test_estimate_tokens_short_string_ascii():
    # M0a-2 weighted: ceil(3/4) == 1
    assert estimate_tokens([{"role": "user", "content": "abc"}]) == 1


def test_estimate_tokens_long_string():
    # ceil(401/4) == 101
    assert estimate_tokens([{"role": "user", "content": "x" * 401}]) == 101


def test_estimate_tokens_cjk_weighted_one_per_char():
    # M0a-2: CJK counts ~1 token/char (not chars//4)
    assert estimate_tokens([{"role": "user", "content": "你好世界"}]) == 4


def test_estimate_tokens_missing_content_key_coerced_to_None_string():
    # str(None) -> "None" → ceil(4/4) == 1
