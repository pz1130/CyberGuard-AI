...
COMPOSE_CHECK_ENV := CYBERGUARD_ENV_FILE=/dev/null POSTGRES_PASSWORD=compose-check-only REDIS_PASSWORD=compose-check-only RUNNER_TOKEN=compose-check-only SKILL_RUNNER_TOKEN=compose-check-only EGRESS_PROXY_TOKEN=compose-check-only BOOTSTRAP_ADMIN_PASSWORD=compose-check-only ENCRYPTION_KEY=$(TEST_ENCRYPTION_KEY) SECRET_KEY=$(TEST_SECRET_KEY)
