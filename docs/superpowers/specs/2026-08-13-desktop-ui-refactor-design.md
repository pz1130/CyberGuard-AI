...
## 13. 遗留与后续（本轮已合并，2026-08-13）

本轮已合入 `main`。以下是**明知未做**的部分，登记在此避免变成无人认领的技术债。

### 13.1 Settings 与 Evidence 仍是旧排布 —— 计划内推迟

按 §2.1 划定的范围，本轮只重做 Workbench。两个视图的现状：

- **被动继承**了新 token（字阶、间距阶、四级层级阶、色彩职责），所以**配色与新界面一致**
- **排布仍是旧的**：`SettingsView.tsx` 1478 行未拆，`EvidenceView.tsx` 未动
- `styles.css` 与 `views.css` 只清了 Workbench 相关规则，它们用的部分按出口判据 4 保留
- `tokens.css` 末尾的兼容层（`--bg` / `--surface` 等旧别名）**是为这两个视图留的**，它们迁完之前不能删

> 下一轮的起点：§5 primitives 清单已标注每个组件对 Settings 的复用度，`Field` 标的是"很高"——
> Settings 是表单密集页，从它入手性价比最高。

### 13.2 「发现」计数靠正则匹配工具输出 —— 需要决策，见 11-OPEN-QUESTIONS §I

`ContextRail` 的 `useFindings()` 用正则从 `tool_call_end` 的 `summary` 文本里抠高危计数：

```
/(\d+)\s*(?:条)?\s*(?:high|critical|高危)/i
```

**这是权宜做法，不严谨**：摘要文案一改就失效，且中英文各写一遍仍覆盖不全。

之所以这么写，是因为当前事件流里**没有结构化的 findings 计数**，而右栏「本次调查的事实」区又必须有内容。
正确解法要 sidecar 侧发结构化事件，属服务端/sidecar 改动，超出本轮（纯 renderer）范围。

**决策已登记为 `docs/desktop/11-OPEN-QUESTIONS.md` §I。** 在它收敛前，改动工具摘要文案的人需要知道这里有隐式耦合。
