    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">管理加密、审计、访问控制等安全策略</p>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">对敏感数据进行静态加密</p>
              </div>
            </div>
            <button onClick={() => update('encryption_enabled', !settings.encryption_enabled)}
              className={`w-12 h-6 rounded-full transition-colors ${settings.encryption_enabled ? 'bg-emerald-500' : 'bg-gray-600'}`}>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">基于角色的权限管理</p>
              </div>
            </div>
            <button onClick={() => update('rbac_enabled', !settings.rbac_enabled)}
              className={`w-12 h-6 rounded-full transition-colors ${settings.rbac_enabled ? 'bg-emerald-500' : 'bg-gray-600'}`}>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">记录所有操作行为，支持 SIEM 导出</p>
              </div>
            </div>
            <button onClick={() => update('audit_logging', !settings.audit_logging)}
              className={`w-12 h-6 rounded-full transition-colors ${settings.audit_logging ? 'bg-emerald-500' : 'bg-gray-600'}`}>
          ...
          {[
            ...
            <div key={key} className="flex items-center justify-between">
              ...
              <input type="number" min={min} max={max}
                ...
                className="w-24 bg-gray-700 rounded-lg px-3 py-1.5 text-sm text-white text-right" />
            </div>
          ))}
        </div>
      </div>
