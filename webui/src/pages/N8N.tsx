        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => setShowGenerator(true)}
            style={{
              ...
              color: '#000', fontWeight: 700, fontSize: 12, letterSpacing: '0.1em', cursor: 'pointer',
              fontFamily: 'var(--font-mono)',
            }}>
            ...
            {connections.map(conn => (
              <div key={conn.id} onClick={() => { setSelectedConn(conn); loadWorkflows(conn) }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: selectedConn?.id === conn.id ? 'var(--accent)' : 'var(--text-primary)', letterSpacing: '0.05em' }}>
                    {conn.name}
                    {conn.is_default && <span style={{ marginLeft: 6, fontSize: 11, color: 'var(--cyan)' }}>DEFAULT</span>}
                  </div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-dim)', marginBottom: 6, wordBreak: 'break-all' }}>{conn.base_url}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                    <div style={{ width: 6, height: 6, borderRadius: '50%', background: conn.is_active ? 'var(--green)' : 'var(--text-dim)' }} />
                    <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>{conn.is_active ? 'ACTIVE' : 'INACTIVE'}</span>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); testConnection(conn) }} disabled={testing}
                    style={{
                      padding: '2px 8px', fontSize: 11, background: 'none', border: '1px solid var(--border-bright)',
                ...
                {testResult && selectedConn?.id === conn.id && (
                  <div style={{ marginTop: 8, padding: '6px 8px', background: testResult.success ? 'var(--green-dim)' : 'var(--red-dim)', border: `1px solid ${testResult.success ? 'var(--green)' : 'var(--red)'}`, fontSize: 11, color: testResult.success ? 'var(--green)' : 'var(--red)' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <input
              ...
              style={{
                width: 200, height: 28, padding: '0 10px', fontSize: 12,
          ...
          {loadingWorkflows ? (
            ...
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-dim)', fontSize: 12, letterSpacing: '0.1em' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {filteredWorkflows.map(wf => (
                <div key={wf.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 4 }}>{wf.name}</div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div style={{ fontSize: 14, fontWeight: 600, letterSpacing: '0.1em', color: 'var(--text-primary)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  <input type="checkbox" checked={connForm.is_default} onChange={e => setConnForm(f => ({ ...f, is_default: e.target.checked }))}
                    style={{ width: 14, height: 14 }} />
                  <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>DEFAULT</span>
                </label>
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <button onClick={() => setShowConnForm(false)} style={{
                  ...
                  color: 'var(--text-muted)', fontSize: 12, letterSpacing: '0.1em', cursor: 'pointer', fontFamily: 'var(--font-mono)',
                }}>CANCEL</button>
                <button onClick={submitConnection} style={{
                  flex: 1, height: 36, background: 'var(--accent)', border: '1px solid var(--accent-border)',
                  color: '#000', fontWeight: 700, fontSize: 12, letterSpacing: '0.1em', cursor: 'pointer', fontFamily: 'var(--font-mono)',
      ...
      {showGenerator && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div style={{ fontSize: 14, fontWeight: 600, letterSpacing: '0.1em', color: 'var(--accent)' }}>
            ...
            {!selectedConn ? (
              <div style={{ padding: 20, textAlign: 'center', color: 'var(--amber)', fontSize: 12, letterSpacing: '0.1em' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  ...
                  <textarea value={genDescription} onChange={e => setGenDescription(e.target.value)}
                    ...
                    style={{
                      ...
                      color: 'var(--text-primary)', fontSize: 14, fontFamily: 'var(--font-mono)', resize: 'vertical',
                    }} />
                </div>
                <button onClick={generateWorkflow} disabled={generating || !genDescription.trim()}
                  style={{
                    ...
                    fontWeight: 700, fontSize: 12, letterSpacing: '0.1em', cursor: generating ? 'not-allowed' : 'pointer',
                ...
                {genError && (
                  <div style={{ padding: '10px 12px', background: 'var(--red-dim)', border: '1px solid var(--red)', fontSize: 12, color: 'var(--red)' }}>
                    ERROR: {genError}
                  </div>
                )}
                {generatedJson && (
                  <div>
                    ...
                    <pre style={{
                      ...
                      fontSize: 12, color: 'var(--text-primary)', maxHeight: 300, overflow: 'auto',
                    ...
                    <button onClick={createWorkflowInN8N} disabled={creatingWorkflow}
                      style={{
                        ...
                        fontSize: 12, letterSpacing: '0.1em', cursor: creatingWorkflow ? 'not-allowed' : 'pointer',
                        fontFamily: 'var(--font-mono)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                      }}>
