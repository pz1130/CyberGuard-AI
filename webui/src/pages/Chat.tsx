...
export default function Chat() {
  ...
  // A run that hits a governance gate suspends on the server and stays that
  // way until a human decides, so it must not look like a run still working.
  // Once approved the same task finishes on its own, so we keep polling —
  // just slowly, because a decision can take an hour.
  ...
  const startPolling = (taskId: string, label: string) => {
    ...
    const tick = async () => {
      ...
      if (task?.status === 'waiting_approval' && !announcedApproval) {
        ...
        // Back off: the next state change needs a person, not a poll.
        clearInterval(pollIntervalRef.current!)
