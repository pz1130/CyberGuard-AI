...
export default function Skills() {
  ...
  return (
    <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <textarea value={configText} onChange={e => setConfigText(e.target.value)}
                rows={4} className="w-full bg-gray-700 rounded-lg px-3 py-2 text-sm text-white font-mono" placeholder="{}" />
            </div>
          </div>
      ...
      {loading ? <p className="text-gray-400">加载中...</p> : items.length === 0 ? <p className="text-gray-500">暂无 Skill</p> : (
            ...
            <div key={s.id} className="bg-gray-800 rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex gap-1">
                  <button onClick={() => { setEditing(s.id!); setForm({ ...s }); setShowForm(true) }} className="p-1.5 text-gray-400 hover:text-white"><Edit2 size={14} /></button>
                  <button onClick={() => del(s.id!)} className="p-1.5 text-gray-400 hover:text-red-400"><Trash2 size={14} /></button>
                </div>
              </div>
