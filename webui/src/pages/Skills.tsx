    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px 0' }}
      ...
      <div style={{ width: 500, maxHeight: 'calc(100vh - 80px)', background: 'var(--bg-surface)', border: '1px solid var(--border-bright)', padding: 24, overflowY: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: '0.1em' }}>{title}</h3>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 9, letterSpacing: '0.2em', color: 'var(--text-dim)', marginBottom: 6 }}>AGENT CAPABILITIES</div>
          <h1 style={{ fontSize: 20, fontWeight: 700, letterSpacing: '0.1em', color: 'var(--text-primary)' }}>SKILL POOL</h1>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={() => setShowInstallUrl(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 14px', height: 36, border: '1px solid var(--border-bright)', background: 'transparent', color: 'var(--text-muted)', fontSize: 11, letterSpacing: '0.1em', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}>
      ...
      {showInstallUrl && (
        <Modal title="INSTALL FROM URL" onClose={() => { setShowInstallUrl(false); setInstallError('') }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.2em', color: 'var(--text-muted)', marginBottom: 6 }}>SKILL FILE URL</label>
              <input value={installUrl} onChange={e => setInstallUrl(e.target.value)}
                placeholder="https://raw.githubusercontent.com/.../skill.md"
                onKeyDown={e => e.key === 'Enter' && handleInstallUrl()}
                style={{ width: '100%', height: 38, padding: '0 12px', background: 'var(--bg-base)', border: '1px solid var(--border-bright)', color: 'var(--text-primary)', fontSize: 12, letterSpacing: '0.05em', fontFamily: 'var(--font-mono)' }} />
            </div>
            {installError && (
              <div style={{ padding: '8px 10px', background: 'rgba(255,0,0,0.1)', border: '1px solid var(--red)', fontSize: 11, color: 'var(--red)', fontFamily: 'var(--font-mono)' }}>
                {installError}
              </div>
            )}
            <div style={{ padding: '8px 10px', background: 'var(--bg-base)', border: '1px solid var(--border)', fontSize: 10, color: 'var(--text-dim)', lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button onClick={() => { setShowInstallUrl(false); setInstallError('') }}
                style={{ padding: '0 14px', height: 36, border: '1px solid var(--border-bright)', background: 'transparent', color: 'var(--text-muted)', fontSize: 11, letterSpacing: '0.1em', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}>
                CANCEL
              </button>
              <button onClick={handleInstallUrl} disabled={installLoading}
                style={{ padding: '0 14px', height: 36, border: '1px solid var(--accent-border)', background: installLoading ? 'var(--bg-elevated)' : 'var(--accent)', color: '#000', fontWeight: 700, fontSize: 11, letterSpacing: '0.1em', cursor: installLoading ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-mono)' }}>
        ...
        <Modal title="IMPORT SKILL FILE" onClose={() => { setShowImport(false); setInstallError('') }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ padding: '20px', background: 'var(--bg-base)', border: '1px dashed var(--border-bright)', textAlign: 'center', cursor: 'pointer' }}
              ...
              <div style={{ fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.05em' }}>Click to select a .md file</div>
              <div style={{ fontSize: 10, color: 'var(--text-dim)', marginTop: 4 }}>or drag and drop</div>
      ...
      {showForm && (
        <div style={{ marginBottom: 24, padding: 24, background: 'var(--bg-surface)', border: '1px solid var(--border-bright)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: '0.1em' }}>{editing ? 'EDIT SKILL' : 'NEW SKILL'}</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.2em', color: 'var(--text-muted)', marginBottom: 6 }}>CATEGORY</label>
              <select value={form.category || 'tool'} onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
                style={{ width: '100%', height: 38, padding: '0 12px', background: 'var(--bg-base)', border: '1px solid var(--border-bright)', color: 'var(--text-primary)', fontSize: 12, fontFamily: 'var(--font-mono)' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.2em', color: 'var(--text-muted)', marginBottom: 6 }}>VERSION</label>
              <input value={form.version || '1.0.0'} onChange={e => setForm(f => ({ ...f, version: e.target.value }))}
                placeholder="1.0.0"
                style={{ width: '100%', height: 38, padding: '0 12px', background: 'var(--bg-base)', border: '1px solid var(--border-bright)', color: 'var(--text-primary)', fontSize: 12, letterSpacing: '0.05em', fontFamily: 'var(--font-mono)' }} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.2em', color: 'var(--text-muted)', marginBottom: 6 }}>PERMISSION</label>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ display: 'block', fontSize: 9, letterSpacing: '0.2em', color: 'var(--text-muted)', marginBottom: 6 }}>MARKDOWN CONTENT</label>
              <textarea value={mdContent} onChange={e => setMdContent(e.target.value)}
                rows={10}
                placeholder={"# Skill Name\n\nDescribe what this skill does..."}
                style={{ width: '100%', padding: '8px 12px', background: 'var(--bg-base)', border: '1px solid var(--border-bright)', color: 'var(--text-primary)', fontSize: 11, letterSpacing: '0.05em', fontFamily: 'var(--font-mono)', resize: 'vertical' }} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 20 }}>
            <button onClick={() => { setShowForm(false); setEditing(null) }}
              style={{ padding: '0 16px', height: 36, border: '1px solid var(--border-bright)', background: 'transparent', color: 'var(--text-muted)', fontSize: 11, letterSpacing: '0.15em', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}>
              CANCEL
            </button>
            <button onClick={submit}
              style={{ padding: '0 16px', height: 36, border: '1px solid var(--accent-border)', background: 'var(--accent)', color: '#000', fontWeight: 700, fontSize: 11, letterSpacing: '0.15em', cursor: 'pointer', fontFamily: 'var(--font-mono)' }}>
      ...
      {!loading && items.length === 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 0', gap: 12 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.1em' }}>NO SKILLS DEPLOYED</div>
          <button onClick={() => setShowForm(true)} style={{ fontSize: 10, color: 'var(--accent)', cursor: 'pointer', background: 'none', border: 'none', letterSpacing: '0.1em' }}>
            + DEPLOY FIRST SKILL
          </button>
        </div>
      )}
              ...
              <div key={s.id} style={{ padding: 20, background: 'var(--bg-surface)', border: '1px solid var(--border-bright)', borderLeft: `3px solid ${tc}` }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', letterSpacing: '0.08em' }}>{s.name}</div>
                  </div>
                  <div style={{ display: 'inline-block', padding: '2px 6px', border: `1px solid ${tc}`, color: tc, fontSize: 8, letterSpacing: '0.15em', background: 'var(--bg-base)' }}>
                    {(s.category || 'tool').toUpperCase()}
                  </div>
                </div>
                <p style={{ fontSize: 10, color: 'var(--text-dim)', lineHeight: 1.6, marginBottom: 8 }}>{s.description || '—'}</p>
                {s.version && (
                  <div style={{ fontSize: 9, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)', marginBottom: 4 }}>v{s.version}</div>
                )}
                <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 9, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)' }}>ID: {String(s.id || '').slice(0, 8) || '—'}</span>
