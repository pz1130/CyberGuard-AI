...
export default function Schedule() {
  ...
  return (
    <div>
      ...
      {showForm && (
        <div className="bg-slate-900 rounded-xl p-6 mb-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs text-slate-400 mb-1 block">Cron 表达式</label>
              <input value={form.cron || ''} onChange={e => setForm(f => ({ ...f, cron: e.target.value }))}
                className="w-full bg-slate-800 rounded-lg px-3 py-2 text-sm text-white" placeholder="0 * * * * (每整点)" />
            </div>
            <div>
              ...
              <input value={form.agent_id || ''} onChange={e => setForm(f => ({ ...f, agent_id: e.target.value }))}
                className="w-full bg-slate-800 rounded-lg px-3 py-2 text-sm text-white" placeholder="可选" />
            </div>
          </div>
      ...
      {loading ? <p className="text-slate-400">加载中...</p> : items.length === 0 ? <p className="text-slate-500">暂无定时任务</p> : (
        <div className="space-y-3">
          {items.map(t => (
            <div key={t.id} className="bg-slate-900 rounded-xl p-4 flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-white">{t.name}</span>
                  <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded">{t.task_type}</span>
                  {t.is_active ? <span className="text-xs bg-violet-500/20 text-violet-400 px-2 py-0.5 rounded">启用</span> : <span className="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded">停用</span>}
                </div>
                {t.cron && <p className="text-xs text-slate-400 mt-1 font-mono">{t.cron}</p>}
                {t.next_run && <p className="text-xs text-slate-500 mt-0.5">下次执行: {t.next_run}</p>}
              </div>
              <div className="flex gap-2">
                <button onClick={() => { setEditing(t.id!); setForm({ ...t }); setShowForm(true) }} className="p-2 text-slate-400 hover:text-white"><Edit2 size={14} /></button>
                <button onClick={() => del(t.id!)} className="p-2 text-slate-400 hover:text-red-400"><Trash2 size={14} /></button>
              </div>
            </div>
          ))}
        </div>
